...
export default {
  ...
  methods: {
    ...
    validateForm() {
      ...
      //event starting date should be greater than or equal to today
      if (this.form.start_date != "") {
        var date = moment(this.form.start_date).add(1, "days");
        var now = moment();
        if (date <= now) {
          this.validatedResult = "failure";
          this.errors.start_date =
            "Event start date must be greater or equal than today";
        }
      }

      //event ending date should be greater or equal than today
      if (this.form.end_date != "") {
        var date = moment(this.form.end_date).add(1, "days");
        var now = moment();
        if (date < now) {
          this.validatedResult = "failure";
          this.errors.end_date =
